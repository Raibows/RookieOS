...
struct SheetControl* sheetcontroll_init(struct MemMan* man, unsigned char* vram, int xsize, int ysize) {
    ...
    for (i=0; i<MAX_SHEETS; ++i) ctl->sheets_pool[i].flags = 0; //标记为未使用
    return ctl;
}
...
void sheet_updown(struct SheetControl* ctl, struct Sheet* sht, int height) {
    ...
    if (old > height) //比以前低
    {
        ...
        sheet_refreshsub(ctl, sht->vx, sht->vy, sht->vx+sht->bxsize, sht->vy+sht->bysize);
    }
...
void sheet_refreshsub(struct SheetControl* ctl, int vx0, int vy0, int vx1, int vy1) {
    int h, bx, by, vx, vy;
    int bx0, bx1, by0, by1;
    unsigned char c;
    for (h=0; h <= ctl->top; ++h)
    {
        ...
        for (by=by0; by<by1; ++by)
        {
            ...
            for (bx=bx0; bx<bx1; ++bx)
            {
                ...
                if (c != ctl->sheets[h]->col_inv) ctl->vram[vy * ctl->xsize + vx] = c;
            }
        }
    }
    return;
...
void sheet_slide(struct SheetControl* ctl, struct Sheet* sht, int vx0, int vy0) {
    ...
    {
        sheet_refreshsub(ctl, old_vx, old_vy, old_vx+sht->bxsize, old_vy+sht->bysize); //原位置的图像进行刷新
        sheet_refreshsub(ctl, vx0, vy0, vx0+sht->bxsize, vy0+sht->bysize); //新位置的图像进行刷新
    }
    return;
}
...
void sheet_free(struct SheetControl* ctl, struct Sheet* sht) {
     ...
     * 释放已使用的图层的内存
     */
    if (sht->height >= 0) sheet_updown(ctl, sht, -1); //先将其隐藏
    sht->flags = 0; //标记未使用
    return;
}
...
void sheet_refresh(struct SheetControl* ctl, struct Sheet* sht, int bx0, int by0, int bx1, int by1) {
    ...
    {
        sheet_refreshsub(ctl, bx0+sht->vx, by0+sht->vy, bx1+sht->vx, by1+sht->vy);
    }
    return;
}
